...
def discretized_mix_logistic_loss(x,l,sum_all=True):
    ...
    nr_mix = int(ls[-1] / (1 + 2*dims)) # here and below: unpacking the params of the mixture of logistics
    print('l shape', ls, 'mix', nr_mix)
    logit_probs = l[:,:,:,:nr_mix]
    l = tf.reshape(l[:,:,:,nr_mix:], xs + [2*nr_mix])
    means = l[:,:,:,:,:nr_mix]
    ...
    means = tf.concat([tf.reshape(means[:,:,:,0,:], [xs[0],xs[1],xs[2],1,nr_mix])],3)
    centered_x = x - means
...
def sample_from_discretized_mix_logistic(l,nr_mix):
    ...
    return tf.reshape(x0,xs[:-1]+[1]) #tf.concat([tf.reshape(x0,xs[:-1]+[1]), tf.reshape(x1,xs[:-1]+[1]), tf.reshape(x2,xs[:-1]+[1])],3)
