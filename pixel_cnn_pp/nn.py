...
dim_mult_1, dim_mult_2, channels = 10, 3, 3
def discretized_mix_logistic_loss(x,l,sum_all=True):
    """ log-likelihood for mixture of discretized logistics, assumes the data has been rescaled to [-1,1] interval """
    xs = int_shape(x) # true image (i.e. labels) to regress to, e.g. (B,32,32,3)
    ls = int_shape(l) # predicted distribution, e.g. (B,32,32,100)
    global dim_mult_1, dim_mult_2, channels
    dim_mult_1, dim_mult_2, channels = num_mult(x)
    ...
    l = tf.reshape(l[:,:,:,nr_mix:], xs + [dim_mult_2 * nr_mix])
    means = l[:,:,:,:,:nr_mix]
    ...
    submeans = [tf.reshape(means[:,:,:,0,:], [xs[0],xs[1],xs[2],1,nr_mix])]
    if channels >= 2:
...
def sample_from_discretized_mix_logistic(l,nr_mix):
    ...
    sub_xs = [tf.reshape(x0,xs[:-1]+[1])]
    if channels >= 2:
        x1 = tf.minimum(tf.maximum(x[:,:,:,1] + coeffs[:,:,:,0]*x0, -1.), 1.)
        sub_xs.append(tf.reshape(x1,xs[:-1]+[1]))
    if channels >= 3:
        x2 = tf.minimum(tf.maximum(x[:,:,:,2] + coeffs[:,:,:,1]*x0 + coeffs[:,:,:,2]*x1, -1.), 1.)
        sub_xs.append(tf.reshape(x2,xs[:-1]+[1]))
    return tf.concat(sub_xs, 3)
